...
/** A score / kill event generated by the simulating side (host or authoritative
 *  server) and mirrored to clients inside the snapshot. Clients consume unseen
 *  ids to show damage-score popups and the kill feed with RELIABLE per-player
 *  attribution (instead of guessing from aggregate score diffs). */
export interface SnapFeedEvent {
  /** monotonically increasing id (per match) — clients keep a watermark */
  id: number;
  kind: "damage" | "kill";
  /** attacker pid / combatant id */
  pid: number;
  /** victim pid for PvP kills; -1 when the victim is an AI enemy */
  victimPid?: number;
  /** display names resolved by the simulating side (fallbacks for bots) */
  killerName?: string;
  victimName?: string;
  weaponId?: string;
  /** score awarded for this event (damage points or kill bounty) */
  amount: number;
  /** attacker's kill count AFTER this kill (kind === "kill" only) */
  kills?: number;
}
...
export interface Snapshot {
  ...
  dmKills?: number[];
  dmTarget?: number;
  /** rolling buffer of recent score/kill events (see SnapFeedEvent) */
  feed?: SnapFeedEvent[];
}
