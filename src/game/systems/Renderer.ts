import { GameEngine, Base, Wall, Player, Enemy, Bullet, EnemyBullet, Particle, Effect, Deployable, Grenade, Pickup, CashBox, Vault, CashoutStation, Statue, Combatant, GridItem, COIN_STYLE, killStyleOf } from "../engine";
...
public render() {
    ...
    this.drawWalls(ctx);
    this.drawDeployables(ctx);
    if ((this.engine.gameMode === "cashout" || this.engine.gameMode === "cashout_5v5")) {
      this.drawCashoutElements(ctx);
    }
    if (this.engine.gameMode !== "biohazard" && this.engine.gameMode !== "cashout" && !this.engine.isDM) {
    ...
    if (this.engine.isDM) {
      ...
      for (const c of this.engine.combatants) {
        ...
        const hasBox = (this.engine.gameMode === "cashout" || this.engine.gameMode === "cashout_5v5") ? this.engine.cashBoxes.find(b => b.carriedByCid === c.id) : null;
        const hasStatue = (this.engine.gameMode === "cashout" || this.engine.gameMode === "cashout_5v5") ? this.engine.statues.find(s => s.carriedByCid === c.id) : null;
...
public drawCashoutElements(ctx: CanvasRenderingContext2D) {
    // Draw Vaults
    for (const v of this.engine.vaults) {
      ctx.save();
    ...
    for (const st of this.engine.cashoutStations) {
      ctx.save();
    ...
    for (const box of this.engine.cashBoxes) {
    ...
    for (const st of this.engine.statues) {
...
public drawOverlays(ctx: CanvasRenderingContext2D) {
    ...
    if ((this.engine.gameMode === "cashout" || this.engine.gameMode === "cashout_5v5") && this.engine.humanInteractProgress > 0) {
      const cx = this.engine.W / 2;
      const cy = this.engine.H / 2 + 55;
      ...
      ctx.fillStyle = "#fbbf24";
      ctx.fillRect(cx - 68, cy - 6, 136 * this.engine.humanInteractProgress, 12);
      ctx.fillStyle = "#ffffff";
      ctx.font = "bold 9px sans-serif";
      ctx.textAlign = "center";
      ctx.fillText(this.engine.humanInteractLabel, cx, cy - 14);
    }
    const p = this.engine.player;
    ...
    // Draw Cashout Station HUD indicators
    if ((this.engine.gameMode === "cashout" || this.engine.gameMode === "cashout_5v5")) {
      this.drawCashoutHUDIndicators(ctx);
    }
  }

  public drawCashoutHUDIndicators(ctx: CanvasRenderingContext2D) {
    const isCashout = (this.engine.gameMode === "cashout" || this.engine.gameMode === "cashout_5v5");
    if (!isCashout) return;
    const teamColors = ["#38bdf8", "#ef4444", "#10b981", "#f59e0b"];
    const p = this.engine.player;
    const margin = 50;

    // Helper: draw a single circular indicator at screen position
    const drawIndicator = (
      wx: number, wy: number,
      label: string, subLabel: string,
      ringColor: string, progress: number,
      iconNumber: string | null,
      timerText: string | null
    ) => {
      const sx = wx - this.engine.camX;
      const sy = wy - this.engine.camY;
      const isOffscreen = sx < margin || sx > this.engine.W - margin || sy < margin || sy > this.engine.H - margin;
      const cx = Math.max(margin, Math.min(this.engine.W - margin, sx));
      const cy = Math.max(margin, Math.min(this.engine.H - margin, sy));
      const distM = Math.round(Math.hypot(wx - p.x, wy - p.y) / 20);
      ...
      // === Outer ring background (dark translucent circle) ===
      ctx.fillStyle = "rgba(10, 15, 25, 0.88)";
      ctx.beginPath();
      ctx.arc(0, 0, 22, 0, Math.PI * 2);
      ctx.fill();

      // === Outer track ring (dim base ring) ===
      ctx.strokeStyle = "rgba(255, 255, 255, 0.15)";
      ctx.lineWidth = 3.5;
      ctx.beginPath();
      ctx.arc(0, 0, 22, 0, Math.PI * 2);
      ...
      // === Progress arc (team-colored, clockwise from top) ===
      if (progress > 0) {
        ctx.strokeStyle = ringColor;
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.arc(0, 0, 22, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * Math.min(1, progress));
        ctx.stroke();
      ...
      if (iconNumber !== null) {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(iconNumber, 0, 1);
      }

      // === Distance text (above indicator) ===
      ...
      ctx.fillText(`${distM}m`, 0, -26);

      // === Sub label (below indicator) ===
      ctx.textBaseline = "top";
      ctx.fillStyle = ringColor;
      ctx.font = "bold 9px sans-serif";
      ctx.fillText(subLabel, 0, 26);

      // === Timer text (below sub label if active) ===
      if (timerText) {
        ctx.fillStyle = "#fde68a";
        ctx.font = "bold 8px sans-serif";
        ctx.fillText(timerText, 0, 37);
      }
    ...
    // --- Draw Vault Indicators ---
    this.engine.vaults.forEach((v, idx) => {
      let vaultColor = "#eab308"; // gold/yellow for vaults
      let prog = 0;
      let timerTxt: string | null = null;
      let subLabel = "VAULT";

      if (v.state === "preheat") {
        subLabel = "VAULT";
        timerTxt = `${Math.ceil(v.timer)}s`;
        vaultColor = "#94a3b8"; // grey when preheating
      } else if (v.state === "unlocking") {
        prog = v.progress ?? 0;
        subLabel = "UNLOCK";
        const tColor = v.unlockingTeamId !== null ? teamColors[v.unlockingTeamId] : "#eab308";
        vaultColor = tColor;
      } else {
        vaultColor = "#eab308";
        subLabel = "VAULT";
    ...
    // --- Draw Cashout Station Indicators ---
    this.engine.cashoutStations.forEach((st, idx) => {
      const stLabel = idx === 0 ? "A" : "B";
      let stColor = st.ownerTeamId !== null ? (teamColors[st.ownerTeamId] || "#94a3b8") : "#94a3b8";
      let prog = st.cashoutProgress ?? 0;
      let subLabel = stLabel;
      let timerTxt: string | null = null;

      if (st.state === "idle") {
        subLabel = stLabel;
        stColor = "#94a3b8";
      } else if (st.state === "cashout") {
        subLabel = `CASH ${stLabel}`;
        timerTxt = `${Math.ceil(st.timer)}s`;
      } else if (st.state === "stealing") {
        subLabel = `STEAL ${stLabel}`;
        timerTxt = `${Math.ceil(st.timer)}s`;
        // Show steal progress on outer ring instead
        prog = st.stealProgress ?? 0;
      } else if (st.state === "settled") {
        subLabel = `DONE ${stLabel}`;
      }
